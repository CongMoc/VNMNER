...
def apply_fixes(filepath='train_umt_pixelcnn_fixedlr_wo_CL.py'):
    ...
    if before_count > after_count:
        fixes_applied.append(f"Replaced {before_count - after_count} instances of label_map[] with label_map.get()")
    ...
    if re.search(dev_pattern, content, re.DOTALL):
        ...
        \2'''
        content = re.sub(dev_pattern, dev_replacement, content, flags=re.DOTALL, count=1)
    ...
    if 'Total predictions collected' not in content:
        ...
        if re.search(dev_report_pattern, content, re.DOTALL):
            content = re.sub(dev_report_pattern, dev_report_replacement, content, flags=re.DOTALL, count=1)
            fixes_applied.append("Added debug info and safety check for dev evaluation")
    ...
    \2'''
        content = re.sub(test_pattern, test_replacement, content, flags=re.DOTALL, count=1)
    ...
    if y_true and y_pred and len(y_true) > 0 and len(y_pred) > 0:
        ...
        if re.search(test_report_pattern, content, re.DOTALL):
            content = re.sub(test_report_pattern, test_report_replacement, content, flags=re.DOTALL)
            fixes_applied.append("Added debug info and safety check for test evaluation")
    ...
    if 'WARNING: No valid predictions found in test set' not in content:
        ...
        if re.search(test_else_pattern, content, re.MULTILINE):
            content = re.sub(test_else_pattern, test_else_replacement, content, flags=re.MULTILINE)
