...
def apply_fixes(filepath='train_umt_pixelcnn_fixedlr_wo_CL.py'):
    ...
    if re.search(dev_report_pattern, content, re.DOTALL):
        content = re.sub(dev_report_pattern, dev_report_replacement, content, flags=re.DOTALL)
    ...
    if re.search(test_simple_pattern, content, re.DOTALL):
        content = re.sub(test_simple_pattern, test_replacement, content, flags=re.DOTALL)
        fixes_applied.append("Added non-empty check and debug info for test evaluation")
